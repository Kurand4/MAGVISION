...
//		if (GetCommTimeouts(KMDport, &lpCommTimeouts)) {
//			lpCommTimeouts.ReadIntervalTimeout 				= 3000;
//			lpCommTimeouts.ReadTotalTimeoutConstant   = 1;
//			lpCommTimeouts.ReadTotalTimeoutMultiplier = 1;
//			SetCommTimeouts(KMDport, &lpCommTimeouts);
//		}
//		m_lLastError = GetLastError();
//		if (m_lLastError) {
...
//---------------------------------------------------------------------------
void __fastcall TSForm::TestGPS(AnsiString s)
{
	COMMTIMEOUTS  lpCommTimeouts;
...
//				lpDCB.BaudRate = 9600;
				...
				lpDCB.Parity   = NOPARITY;
				lpDCB.StopBits = ONESTOPBIT;
				SetCommState(GPSport, &lpDCB);
			}
			m_lLastError = GetLastError();
		} else {
			GPSport = NULL;
			GPSMemo->Lines->Add("No data");
			return;
		}
	}
	FlushFileBuffers(GPSport);
	ReadFile(GPSport, buf, sizeof(&buf), &BytesRead, NULL);
	s = "";
	for (DWORD i = 0; i < BytesRead; i++) s += buf[i];
	GPSMemo->Lines->Add(s);
}
//---------------------------------------------------------------------------
void __fastcall TSForm::GPSBthButtonClick(TObject *Sender)
{
	GPSBthButton->Enabled = false;
	TestGPS(CBox_GPSBth->Text);
	GPSBthButton->Enabled = true;
	Application->ProcessMessages();
}
//---------------------------------------------------------------------------
void __fastcall TSForm::CBox_KMDBthCloseUp(TObject *Sender)
{
	BthKMDname = CBox_KMDBth->Items->Strings[CBox_KMDBth->ItemIndex];
}
//---------------------------------------------------------------------------
...
{
	ShellExecute(h,"open", "C:\\windows\\system32\\rundll32.exe","shell32.dll,Control_RunDLL bthprops.cpl,,2",0,SW_SHOW);
}
//---------------------------------------------------------------------------
void __fastcall TSForm::AddBluetoothButtonClick(TObject *Sender)
{
	ShellExecute(h,"open", "C:\\windows\\system32\\rundll32.exe","shell32.dll,Control_RunDLL bthprops.cpl",0,SW_SHOW);
}
//---------------------------------------------------------------------------
...
{
	sDevNum = CB_DevNum->ItemIndex;
}
//---------------------------------------------------------------------------


void __fastcall TSForm::CBox_BGrndClick(TObject *Sender)
{
	MainForm->UseBGrndFlag = CBox_BGrnd->Checked;
}
//---------------------------------------------------------------------------
